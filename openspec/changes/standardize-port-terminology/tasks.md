...
- [ ] 1.1 Rename `FeatureModule` class to `DomainModule` in `src/core/feature-module.base.ts`
...
- [ ] 2.5 Update `examples/storage/object-storage.module.ts` to use `DomainModule` and `@ExportsService`
- [ ] 2.6 Update `examples/storage/adapters/s3/s3.adapter.ts` to use `@ProvidesPort`
- [ ] 2.7 Update `examples/rates/currency-rates.module.ts` to use `DomainModule` and `@ExportsService`
...
- [ ] 3.2 Replace all instances of `FeatureModule` with `DomainModule`
